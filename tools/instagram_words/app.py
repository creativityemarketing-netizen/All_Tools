...
RUNTIME_DIR = tool_data_dir("instagram_words")
...
DEFAULT_COOKIE_FILE = os.getenv("INSTAGRAM_COOKIE_FILE", "")
DEFAULT_SCAN_LIMIT = 120
DEFAULT_MATCH_LIMIT = 30
DEFAULT_SEARCH_SECONDS = 240
INSTAGRAM_REQUEST_TIMEOUT = int(os.getenv("INSTAGRAM_REQUEST_TIMEOUT", "18"))
INSTAGRAM_CACHE_TTL = int(os.getenv("INSTAGRAM_CACHE_TTL", str(24 * 60 * 60)))
...
def cache_key(username: str, source: str) -> Path:
    safe_user = re.sub(r"[^A-Za-z0-9_.-]+", "_", username.casefold().strip())
    safe_source = "reels" if source == "reel" else "posts"
    return CACHE_DIR / f"{safe_user}-{safe_source}.json"


def read_cached_source(username: str, source: str) -> dict | None:
    path = cache_key(username, source)
    if not path.exists():
        return None
    try:
        ...
        return None
    fetched_at = compact_number(data.get("fetched_at"))
    if INSTAGRAM_CACHE_TTL and fetched_at and time.time() - fetched_at > INSTAGRAM_CACHE_TTL:
        return None
...
def write_cached_source(username: str, source: str, items: list[dict], complete: bool) -> None:
...
def cached_sources_for_area(content_area: str) -> tuple[str, ...]:
    if content_area == "posts":
        return ("post",)
    if content_area == "reels":
        return ("reel",)
    return ("post", "reel")


def read_cached_content(username: str, content_area: str, scan_limit: int = 0) -> tuple[list[dict], bool]:
    items: list[dict] = []
    all_usable = True
    for source in cached_sources_for_area(content_area):
        cached = read_cached_source(username, source)
        if not cached:
            all_usable = False
            continue
        source_items = cached.get("items", [])
        if not cached.get("complete") and (not scan_limit or len(source_items) < scan_limit):
            all_usable = False
            continue
        items.extend(source_items)
    items.sort(key=lambda item: item.get("date_utc") or "", reverse=True)
    if scan_limit:
        items = items[:scan_limit]
    return items, bool(items) and all_usable


def content_payload(post: instaloader.Post, source: str = "post") -> dict:
...
def cached_post_payload(item: dict, matched_words: list[str]) -> dict:
    payload = dict(item)
    payload.pop("date_utc", None)
    payload["matched_words"] = matched_words
    return payload
...
def post_payload(post: instaloader.Post, matched_words: list[str], source: str = "post") -> dict:
    return cached_post_payload(content_payload(post, source), matched_words)
...
def search():
    ...
    try:
        ...
        scanned = 0
        matches: list[dict] = []
        cached_items, cache_hit = read_cached_content(username, content_area, max_posts)
        if cache_hit:
            scanned = len(cached_items)
            for item in cached_items:
                matched_words = caption_matches(item.get("caption", ""), words, mode)
                if matched_words:
                    matches.append(cached_post_payload(item, matched_words))
        else:
            per_source_limit = max(1, max_posts // 2) if max_posts and content_area == "both" else max_posts
            collected_by_source: dict[str, list[dict]] = {"post": [], "reel": []}
            stopped_by_limit = False
            for source, post in iter_profile_content(profile, per_source_limit, content_area):
                scanned += 1
                base_item = content_payload(post, source)
                collected_by_source.setdefault(source, []).append(base_item)
                caption = base_item.get("caption", "")
                matched_words = caption_matches(caption, words, mode)
                if matched_words:
                    matches.append(cached_post_payload(base_item, matched_words))
                if max_posts and scanned >= max_posts:
                    stopped_by_limit = True
                    break
            for source, items in collected_by_source.items():
                if items:
                    write_cached_source(username, source, items, complete=not stopped_by_limit and (content_area in (source + "s", "both") or content_area == source))

        cache_status = "hit" if cache_hit else "refreshed"
...
def stream_search_posts(
    ...
    try:
        ...
        cached_items, cache_hit = read_cached_content(username, content_area, scan_limit)
        if cache_hit:
            result["cache"] = "hit"
            for item in cached_items:
                scanned += 1
                ...
                matched_words = caption_matches(item.get("caption", ""), words, mode)
                if matched_words:
                    found = cached_post_payload(item, matched_words)
                    matches.append(found)
                    yield sse("item", {"job_id": job_id, "item": found, "matches": len(matches), "scanned": scanned, "source": current_source, "cache": True})
                elif scanned == 1 or scanned % 50 == 0:
                    yield sse("progress", {"job_id": job_id, "matches": len(matches), "scanned": scanned, "source": current_source, "cache": True})
                if match_limit and len(matches) >= match_limit:
                    stop_reason = "match_limit"
                    break
            yield sse("done", {"job_id": job_id, "matches": len(matches), "scanned": scanned, "source": current_source, "reason": stop_reason, "cache": True})
            return
        ...
        for source, post in iter_profile_content(profile, per_source_limit, content_area):
            ...
            matched_words = caption_matches(caption, words, mode)
            if matched_words:
                item = cached_post_payload(base_item, matched_words)
                matches.append(item)
        ...
        for source, items in collected_by_source.items():
            if items:
                write_cached_source(username, source, items, complete=stop_reason == "finished")
