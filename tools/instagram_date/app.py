...
RUNTIME_DIR = tool_data_dir("instagram_date")
...
SHORTCODE_RE  = re.compile(r"instagram\.com/(?:p|reels?|tv)/([A-Za-z0-9_-]+)")
...
LAST_DB_FILE = RUNTIME_DIR / ".last_database"   # remembers which file was last used
...
def auto_load():
    ...
    if LAST_DB_FILE.exists():
        ...
        if not remembered.is_absolute():
            remembered = BASE_DIR / remembered
