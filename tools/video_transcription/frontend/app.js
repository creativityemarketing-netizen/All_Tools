...
function startTimer(message, durationSeconds = 0) {
  ...
  function tick() {
    ...
    processNote.textContent = `${message} Elapsed ${formatShortTime(elapsed)}. About ${formatShortTime(remaining)} left.`;
  }
...
function updateMediaInfo(info = {}) {
  ...
  if (info.mediaUrl) {
    if (info.thumbnail) {
      ...
      image.onerror = () => {
        button.classList.add("poster-failed");
        image.remove();
        if (!button.querySelector("span")) {
          const fallback = document.createElement("span");
          fallback.textContent = "Click to preview video";
  ...
  if (info.thumbnail) {
    ...
    image.onerror = () => {
      mediaPreview.innerHTML = "";
      const fallback = document.createElement("span");
      fallback.textContent = "Preview image blocked by platform";
  ...
  const fallback = document.createElement("span");
  fallback.textContent = info.previewMessage || "Preview unavailable";
  mediaPreview.append(fallback);
}
